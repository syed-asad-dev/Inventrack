...
const SupplierEntry = () => {
  ...
  const [formData, setFormData] = useState({ recordId: 'Auto-generated', supplierName: '' });
  ...
  const fetchData = async () => {
    try { setLoading(true); const res = await axios.get(`${BASE_URL}/api/suppliers`); setData(Array.isArray(res.data) ? res.data : []); }
    catch { toast.error('Failed to fetch suppliers'); } finally { setLoading(false); }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.supplierName.trim()) { toast.error('Supplier Name is required'); return; }
    try {
      setSaving(true);
      if (isEditing) { await axios.put(`${BASE_URL}/api/suppliers/${editId}`, { supplierName: formData.supplierName }); toast.success('Supplier updated'); }
      else { await axios.post(`${BASE_URL}/api/suppliers`, { supplierName: formData.supplierName }); toast.success('Supplier created'); }
      resetForm(); fetchData();
    } catch (error) { toast.error(error.response?.data?.error || 'An error occurred'); } finally { setSaving(false); }
  };

  const editItem = (item) => { setIsEditing(true); setEditId(item._id); setFormData({ recordId: item.recordId, supplierName: item.supplierName }); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const resetForm = () => { setIsEditing(false); setEditId(null); setFormData({ recordId: 'Auto-generated', supplierName: '' }); };
  const confirmDelete = (item) => { setItemToDelete(item); setDeleteModalOpen(true); };

  const handleDelete = async () => {
    try { await axios.delete(`${BASE_URL}/api/suppliers/${itemToDelete._id}`); toast.success('Supplier deleted'); fetchData(); }
  ...
  const filtered = data.filter(d => d.supplierName.toLowerCase().includes(searchTerm.toLowerCase()) || d.recordId.toLowerCase().includes(searchTerm.toLowerCase()));

  return (
    <div className="space-y-8 animate-fade-in">
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
          <h2 className="text-lg font-bold text-gray-800 uppercase tracking-wide">{isEditing ? 'EDIT SUPPLIER' : 'SUPPLIER ENTRY'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div><label className="block text-sm font-medium text-gray-700 mb-2">Supplier Name <span className="text-red-500">*</span></label><input type="text" value={formData.supplierName} onChange={(e) => setFormData({...formData, supplierName: e.target.value})} placeholder="e.g. ABC Traders" required className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" /></div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
          <h3 className="text-lg font-bold text-gray-800">Suppliers List</h3>
        ...
        <div className="overflow-x-auto">
          {loading ? <div className="flex justify-center p-12 text-blue-600"><Loader2 size={40} className="animate-spin" /></div> : filtered.length > 0 ? (
            <table className="w-full text-left"><thead><tr className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider"><th className="px-6 py-4 font-semibold">ID</th><th className="px-6 py-4 font-semibold">Supplier Name</th><th className="px-6 py-4 font-semibold text-right">Actions</th></tr></thead>
              <tbody className="divide-y divide-gray-100">{filtered.map(item => (
                <tr key={item._id} className="hover:bg-gray-50/50"><td className="px-6 py-4 text-sm text-gray-600 font-medium">{item.recordId}</td><td className="px-6 py-4 text-sm font-bold text-gray-800">{item.supplierName}</td>
          ...
          ) : <div className="p-12 text-center text-gray-500">{searchTerm ? 'No match.' : 'No suppliers found.'}</div>}
        </div>
      </div>
      ...
      {deleteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in"><div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
          ...
          <h3 className="text-xl font-bold text-center text-gray-900 mb-2">Delete Supplier</h3>
          <p className="text-center text-gray-500 mb-6">Delete <span className="font-bold text-gray-800">{itemToDelete?.recordId} ({itemToDelete?.supplierName})</span>?</p>
